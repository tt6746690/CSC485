...
void ip_cksum(const uint8_t *buf) 
{ 
   sr_ip_hdr_t *iphdr = (sr_ip_hdr_t *)(buf);
   iphdr->ip_sum = 0;
   iphdr->ip_sum = cksum(buf, sizeof(sr_ip_hdr_t));
}
void icmp_cksum(const uint8_t *buf, int len) { 
    sr_icmp_hdr_t *icmphdr = (sr_icmp_hdr_t *)(buf);
    icmphdr->icmp_sum = 0;
    icmphdr->icmp_sum = cksum(buf, len);
}

void icmp_t3_cksum(const uint8_t *buf, int len) { 
    sr_icmp_t3_hdr_t *icmphdr = (sr_icmp_t3_hdr_t *)(buf);
    icmphdr->icmp_sum = 0;
    icmphdr->icmp_sum = cksum(buf, len);
}

...
uint16_t compute_tcp_cksum(uint8_t* packet, unsigned int len) 
{
    DECL_IP(packet, len);
    DECL_TCP(packet, len);

    int tcp_len = ntohs(iphdr->ip_len) - IP_HDR_SIZE;
    /* assert(ETHIP_HDR_SIZE + tcp_len == len); [> might cause premature termination <] */

    /* ------        --------       ---------   (buf_len)
     * pseudo hdr     tcp hdr       tcp payload */
    int buf_len = TCP_PSEUDO_HDR_SIZE + tcp_len;
    uint8_t* buf = (uint8_t*)malloc(buf_len);
    memcpy(buf + TCP_PSEUDO_HDR_SIZE, packet + ETHIP_HDR_SIZE, tcp_len);

    sr_tcp_pseudo_hdr_t* pshdr = (sr_tcp_pseudo_hdr_t*)buf;
    pshdr->ip_src   = iphdr->ip_src;
    pshdr->ip_dst   = iphdr->ip_dst;
    pshdr->reserved = 0;
    pshdr->protocol = iphdr->ip_p;
    pshdr->tcp_len  = htons(tcp_len);

    /* compute tcp cksum */
    uint16_t sum = cksum(buf, buf_len);
    free(buf);
...
void tcp_cksum(uint8_t *packet, int len) { 
    DECL_TCP(packet, len);
    tcphdr->checksum = 0;
    tcphdr->checksum = compute_tcp_cksum(packet, len);
}
        ...
        char* interface/* lent */)
{
    ...
    if(ETHTYPE_IS_IP(ethhdr)) {
        ...
        if(TO_THIS_ROUTER) {
            if(IPP_IS_ICMP(iphdr)) {
                ...
                if (SR_NAT_ON(sr)) {
                    ...
                    if (IF_INBOUND(interface)) {
                        FMT("\tICMP Inbound\n");
                        mapping = sr_nat_lookup_external(&sr->nat, icmp3hdr->unused, nat_mapping_icmp);
                        if(!mapping) return;
                        iphdr->ip_dst    = mapping->ip_int;
                        icmp3hdr->unused = mapping->aux_int;
                        free(mapping);
                    } else { NO_REACH return; }

                    icmp_t3_cksum(icmp3pkt, len - (icmp3pkt - packet));
                    ip_cksum(ippkt);
                ...
                if (SR_NAT_ON(sr) && IPP_IS_TCP(iphdr)) {
                    DECL_TCP(packet, len);
            ...
            /* check if packet is outbound(eth1) or inbound(eth2)
             *      outbound: lookup/insert NAT matting
             *      inbound : drop packet if no mapping and not a SYN
             * Rewrite IP src (dst) for outgoing(incoming) packet 
             * Update ICMP ID / TCP port
             * Update cksum
             * Route packet
             */
            if (SR_NAT_ON(sr)) {
                if (IPP_IS_ICMP(iphdr)) {
                    DECL_ICMP3(packet, len);
                    CHECK_CKSUM(icmp3pkt, len-(icmp3pkt-packet));

                    sr_nat_mapping_t* mapping; 
                    if (IF_OUTBOUND(interface)) {
                        FMT("\tICMP Outbound\n");
                        mapping = sr_nat_insert_mapping(&sr->nat, iphdr->ip_src, icmp3hdr->unused, nat_mapping_icmp);
                        iphdr->ip_src    = mapping->ip_ext;
                        icmp3hdr->unused = mapping->aux_ext;
                        free(mapping);
                    } else { NO_REACH return; }

                    icmp_t3_cksum(icmp3pkt, len - (icmp3pkt - packet));
                } else if (IPP_IS_TCP(iphdr)) {
                    DECL_TCP(packet, len);
                    CHECK_TCP_CKSUM(packet, len);
                    ...
                    tcp_cksum(packet, len);
                } else if (IPP_IS_UDP(iphdr)) { NO_REACH return; }
            }

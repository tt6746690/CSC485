...
#define TCP_HDR_SIZE (sizeof(sr_tcp_hdr_t))
#define TCP_PSEUDO_HDR_SIZE (sizeof(sr_tcp_pseudo_hdr_t))
...
#define DECL_PKT_TCP(BUF, LEN)      DECL_PKT_HDR(tcp,   BUF, LEN, sr_tcp_hdr_t)
...
#define DECL_TCP(BUF, LEN)      DECL_PKT_TCP(BUF+ETHIP_HDR_SIZE, LEN-ETHIP_HDR_SIZE)
...
/* Note buf here is packet */
#define CHECK_TCP_CKSUM(BUF, LEN) \
    do { \
        if(compute_tcp_cksum(BUF, LEN)) { fprintf(stderr, #BUF " packet (length %d) has incorrect checksum %d\n", LEN, compute_tcp_cksum(BUF, LEN)); } \
    } while(0)
...
/* macro for working with interfaces */
#define IF_IS(interface, name) (!strcmp(interface, name))
...
#define SR_NAT_ON(sr) (((sr)->nat).nat_on)

...
int sr_handle_hwinfo(struct sr_instance* sr, c_hwinfo* hwinfo)
{
    ...
    /* sets ip_ext for nat table */
    sr_if_t* iface = sr_get_interface(sr, "eth2");
    sr->nat.ip_ext = iface->ip;
    /* print_addr_ip_int(ntohl(sr->nat.ip_ext)); */
                                ...
                                const char* name /* borrowed */ )
{
    ...
    ether_hdr = (struct sr_ethernet_hdr*)buf;   /* dest,src MAC + ether_type (14 bytes) */
    ...
    /* ether_shost is the source MAC addr 
     simply check buf's source MAC matches with that of an sr_if associated with sr */
                         ...
                         const char* iface /* borrowed */)
{
    ...
    /* don't waste my time ... 
        sizeof(sr_ethernet_hdr) == 14 bytes */
    if ( len < sizeof(struct sr_ethernet_hdr) ){
        fprintf(stderr , "** Error: packet is wayy to short \n");
        return -1;
    }

    /* Create packet, by populating c_packet_header */
    ...
    /* copy over data inside buffer to packet being constructed */
    ...
    /* send packet over network with sr's socket */
...
void sr_log_packet(struct sr_instance* sr, uint8_t* buf, int len )
{
    ...
    gettimeofday(&h.ts, 0);         /* time stamp now */
    h.caplen = size;

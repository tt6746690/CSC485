/* This file defines an ARP cache, which is made of two structures: 
    * an ARP request queue
        holds data about an outgoing ARP cache request and 
        the packets that are waiting on a reply to that ARP cache request.
        (multiple packets can wait on a single ARP request if their dest IPs are same)
    * ARP cache entries. 
        hold IP->MAC mappings and are timed out every SR_ARPCACHE_TO seconds.
   ...
   if entry:
       ...
       free entry
        ...
        // entry = NULL, no malloc arpcache_lookup
        // Now stores raw ethernet frame in the queue, waiting to be sent 
        // once ARP request reply gets the destination MAC addr
...
struct sr_arpentry {            /* IP -> MAC pair */
    unsigned char mac[6]; 
...
struct sr_arpreq {
    uint32_t ip;                /* query IP (want to know its associated MAC) */
...
struct sr_arpcache {
    struct sr_arpentry entries[SR_ARPCACHE_SZ]; /* 100 cache entries, contains IP -> MAC mapping */
    struct sr_arpreq *requests;                 /* linked-list of requests */
 ...
 *
   ...
   if req:
       ...
       arpreq_destroy(req)
 */
void handle_arpreply(struct sr_instance *sr, uint8_t *mac, uint32_t ip);


/* Handles procedure for  
 *      sending arp request on request if it is at least 1s old (from creation)
 *      destroying arp request and send corres ICPM if it is too old (5 arp req sent already)
 */
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq* req);


/* Checks if an IP->MAC mapping is in the cache. IP is in network byte order. 
   You must free the returned structure if it is not NULL. (returns a copy of sr_arpentry)*/

...
int main(int argc, char **argv)
{
    ...
    /* default timeout values for nat table */
    sr.nat.timeout_icmp = DEFAULT_ICMP_TIMEOUT;
    sr.nat.timeout_tcp_established = DEFAULT_TCP_ESTABLISHED_IDLE_TIMEOUT;
    sr.nat.timeout_tcp_transitory = DEFAULT_TCP_TRANSITORY_IDLE_TIMEOUT;
    ...
    while ((c = getopt(argc, argv, "hns:v:p:u:t:r:l:T:I:E:R:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'T':
                ...
                break;
            case 'I':
                sr.nat.timeout_icmp = atoi((char *) optarg);
                break;
            case 'E':
                sr.nat.timeout_tcp_established = atoi((char *) optarg);
                break;
            case 'R':
                sr.nat.timeout_tcp_transitory = atoi((char *) optarg);
                break;
        } /* switch */
    } /* -- while -- */

    sr.nat.nat_on = nat_on;

    FMT("opts nat_on=%d I=%d E=%d R=%d\n", 
            sr.nat.nat_on, sr.nat.timeout_icmp, sr.nat.timeout_tcp_established, sr.nat.timeout_tcp_transitory);
    ...
    if(! user )
    { sr_set_user(&sr); }       /* in vm, user=mininet */
    else
